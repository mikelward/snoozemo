...
one to be fixed and the others not. **One settling value, and one line: `Waiting for location`
for the whole capture** (maintainer, 2026-09-08). It was briefly two. The objection that produced
the split stands and is worth keeping: with location denied or services off the capture records
"no fix" at once and waits on Wi-Fi or the ceiling, so this line spends that window naming a
sensor that is not coming. The split answered it with a second settling value and a second line,
`Waiting for Wi-Fi`, shown once only the SSID read was left — and keyed on which half was still
*outstanding* rather than which had failed, since a cause covers only the failures somebody
enumerated and misses the ordinary case where a fix simply arrives first.

What that bought in practice was a flicker: the Wi-Fi read is near-instant, so the second line
appeared for a fraction of a second on the way to the real content, and two sentences flickering
past read worse than one that stands until the card fills in. So the objection is **accepted
...
- **Active:** label `Snoozing`; the subtitle names **how the snooze ends** — the end time itself for
  ...
  **absolute end time, not a countdown**, so the subtitle never shows a stale figure — the tile
  ...
  not expect to reach (the countdown rule below). **Wi-Fi grace is the one watched state that names
  neither** — `Ending soon`, ahead of the leave/move exits, because grace is a short deadline that
  ends the snooze on expiry whether or not the phone leaves or moves, so an exit here could be
  disproved minutes later; the screen and notification say `Wi-Fi lost — ending soon` for the same
  reason. A distance-to-go readout for the departure exit
...
type roles, above every other line of content, and sits in the middle where a glance lands. The
app's own name in the title row stays larger still: this is the loudest thing the screen *says*,
not the loudest thing on it. Idle says `Not snoozing`;
