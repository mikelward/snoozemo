 ...
 * geofence delivers (`SPEC.md` §6.10). The `direct` flavor's own copy answers
 * `false`: it runs duration-only snoozes until Phase 7, so it has nothing to
 * outlive and deliberately declares none of those (`DeclaredPermissionsTest`).
 *
 * **What reads it is `When I move` (`SPEC.md` §4.4.)** That exit needs the
 * process kept alive to hear a one-shot sensor, and on a flavor with no
 ...
 * exposed `direct` and made this its own question (Codex, PR #252).
 *
