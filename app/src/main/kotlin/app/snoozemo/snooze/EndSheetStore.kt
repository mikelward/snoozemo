...
internal class EndSheetStore(context: Context) {
    ...
    fun isEnabled(): Boolean {
        // Publish to the shared [ChooserMode] cache only if no write landed while
        // we were reading disk. A slow warm-up read could otherwise read the old
        // value, be preempted by `setEnabled` committing and caching the new one,
        // and then overwrite it back to the stale value — so the next tile tap in
        // this still-live process takes the opposite route to the toggle the user
        // just made (Codex, PR #284). The generation is bumped by every write, so
        // a read that sees it unchanged knows its value is not stale.
        val seenGeneration = ChooserMode.beginRead()
        val value = prefs.getBoolean(KEY_ENABLED, false)
        afterReadBeforePublishForTest?.invoke()
        ChooserMode.publishIfUnchanged(value, seenGeneration)
        return value
    }
    ...
    fun setEnabled(enabled: Boolean): Boolean = prefs.write {
        // A raw read, deliberately *not* [isEnabled] — that publishes the value
        // it reads, which would overwrite an optimistic publish
        // [EndSheetSetting] made on the caller's thread (below) with the old disk
        // value mid-write, reopening the window this closes (Codex, PR #284).
        val before = prefs.getBoolean(KEY_ENABLED, false)
        val persisted = prefs.putBoolean(KEY_ENABLED, enabled)
        if (!persisted) {
            prefs.putBoolean(KEY_ENABLED, before)
        }
        // The value now in force, published with a bumped generation so a
        // concurrent [isEnabled] read cannot clobber it with a stale value it
        // read before this write (see [isEnabled] and [ChooserMode]). On a
        // refused write this rolls the optimistic publish back to `before`.
        ChooserMode.publish(if (persisted) enabled else before)
        persisted
    }
    ...
    internal companion object {
        ...
        /**
         * The warmed value, or `false` until the warm-up has landed — the shared
         * [ChooserMode] cache the tile reads too. See that type for why it lives
         * in `:core` and defaults to off (the arm-preserving fallback).
         */
        fun cachedEnabled(): Boolean = ChooserMode.isOn()

        /** Drops the cache, so a test cannot inherit another's warmed value. */
        @VisibleForTesting
        internal fun resetCacheForTest() {
            ChooserMode.resetForTest()
        }
