...
open class SnoozeService : Service(), SnoozeController.Listener {
    ...
    private fun reconcileRuleStatus(ruleId: String?, status: Int) {
        ...
        // Read back what the rule is doing now, because the broadcast describes
     ...
     * failure means the drift of §5.8 is never noticed at all rather than
    ...
    override fun onReleasing(reason: EndReason) {
        ...
        // A refused `commit()` returns false rather than throwing, so the
        // result is inspected the way the clear path already does (Codex, PR
        // #36). What a missing marker costs is attribution, not the release: a
        ...
        if (!marked) {
            SnoozeDebugLog.warning(
                "release reason not recorded; a crash before cleanup would misread this ending",
            )
        }
    }
    ...
    private fun armAsAsked(intent: Intent?): EndChoiceResult {
        ...
        // **Carried, not discarded.** An exit that would not come off is a
        // partial success here exactly as it is over a running snooze, and the
        // answer is the only thing that says so at the tap now that no card
        // does. Dropping it reported a clean apply over a snooze that could
        // still end early.
        var narrowing = EndChoiceResult.APPLIED
        if (choseATime) {
            controller.active?.let { running ->
                ...
                narrowing = makeTimerOnly(running, restoring = false)
            }
        }

        if (intent?.getBooleanExtra(EXTRA_ENDS_ON_MOTION, false) != true) return narrowing
        ...
        val snooze = controller.active ?: return narrowing
    ...
    private fun setMotionEnd(intent: Intent?) {
        // The previous attempt's card comes down on the **outcome**, not as
        // this one starts — see [setCap], where three rounds put a cancel at a
        // position in the sequence and each position was wrong in its own way.
        // It lives inside [applyMotionEndChoice], past that function's
        // validation and keyed to `APPLIED`: a stale intent for a snooze that
        // has already ended cannot take down the running snooze's card on its
        // way to `GONE`, and a `REFUSED` leaves a warning that is still true
        // standing rather than deleting it with nothing to put in its place.
    ...
    private fun applyMotionEnd(
        ...
        // `APPLIED` only.
        //
        // Whether this tap is what turned the motion exit on — as opposed to a
        // retry over an exit an earlier choice already armed, where the add above
        // was skipped. Only an exit this tap added is this tap's to undo.
        val addedExit = wanted && !snooze.endsOnMotion
        ...
        // `APPLIED` only.
        //
        // **A clear that will not persist un-applies the tap that added the exit.**
        // Reporting `APPLIED` over a record that still carries the intent would
        // leave the failure-styled "Set, but this can still end sooner" line
        // standing over a motion exit the user deliberately added, and carry it
        // across a restart (Codex, PR #272; maintainer chose this over accepting
        // it, 2026-09-12). So a failed clear rolls the exit back to the pre-tap
        // state — the intent stands, no exit is armed, nothing half-applied is
        // durable — and the tap is reported `REFUSED`, so the row shows it did not
        // take and the user re-taps.
        //
        // **Only an exit this tap added is rolled back.** A retry over a motion
        // exit an earlier choice already armed (`!addedExit`) must not roll that
        // exit off on a failed clear — that would silently remove an exit the user
        // still wants and leave the phone quiet to the cap (Codex, PR #272). There,
        // and in the doubly-rare case where the rollback write itself will not
        // persist, the exit stays and the tap is `APPLIED`: the exit is real, and
        // an extra warning beats discarding it (D7 — still no silence either way).
        if (clearTimerOnly && result == EndChoiceResult.APPLIED) {
            val running = controller.active
            if (running != null && running.timerOnlyRequested) {
                when {
                    updateRecordOrUndo(running.copy(timerOnlyRequested = false), running) ->
                        controller.setTimerOnlyRequested(false)
                    // The record first, then the controller — the ordering the
                    // whole path keeps; `setEndsOnMotion(false)` reconciles the
                    // watch closed.
                    addedExit && updateRecordOrUndo(running.copy(endsOnMotion = false), running) -> {
                        controller.setEndsOnMotion(false)
                        SnoozeDebugLog.warning("until-i-move: intent clear failed; rolled the new exit back, tap refused")
                        return EndChoiceResult.REFUSED
                    }
                    else ->
                        SnoozeDebugLog.warning("until-i-move: intent clear failed; exit kept, intent stands")
                }
            }
        }
        return result
    ...
    private fun applyChosenEnd(intent: Intent?): EndChoiceResult {
        ...
        val snooze = if (restoring && exitBase.endsOnMotion) {
            if (applyMotionEnd(exitBase, wanted = false) != EndChoiceResult.APPLIED) {
                ...
                // `REFUSED`, and the cap restore below has not run yet — so
                // this leaves the snooze ending *earlier* than the user asked,
                // which is the safe direction and a retry away. What it still
                // ends on is on the ongoing card, which names every armed exit
     ...
     * sooner. So it answers [EndChoiceResult.PARTIAL] rather than `APPLIED` or
     * `REFUSED` — the sheet stays up saying the snooze can still end sooner,
     * where `APPLIED` would dismiss it exactly as a clean apply does and
     * `REFUSED` would describe the opposite failure. **Which** exit survived is
     * not carried with it: the ongoing card is rebuilt from the record on every
     * post and already names every armed exit, so that half is derived and
     * cannot fall out of step.
     *
     * **The shade's own card is cleared here**, on the choice applying at all:
     * the previous attempt's `Couldn't set the end time` is stale whether or
     * not an exit stayed armed, because the time did take. See [setCap] for
     * what cancelling by position cost.
     */
    private fun makeTimerOnly(snooze: ActiveSnooze, restoring: Boolean): EndChoiceResult {
        ...
        notifications.cancelFailure()
        // **What stayed armed is answered, not posted** (maintainer,
        // 2026-09-12). A card of its own had to be retired by every operation
        // that removes an exit, ends a snooze or replaces one, and nine review
        // findings in a row landed on getting that set of sites right. It also
        // could not be seen at all where notifications are denied. So the
        // durable half now rides the ongoing card, which is rebuilt from the
        // record on every transition and therefore names exactly the exits the
        // record still carries — it cannot go stale, and there is nothing to
        // retire. This answers the *tap*, which is what the sheet says at the
        // moment the user makes it.
        return if (movementStayed || departureStayed) {
...
/**
 * The platform's status int as the domain's [ZenRuleStatus] (SPEC.md §5.8).
 *
