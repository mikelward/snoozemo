...
anchor supports nothing better; `direct` is duration-only until Phase 7. Phase 3's remaining
code is blocked on decisions, not effort — the degradation *cause* reaching the user needs
approved copy, and §6.7's resting poll needs a battery call — alongside the on-device
verification (`TODO.md` Phase 3).
