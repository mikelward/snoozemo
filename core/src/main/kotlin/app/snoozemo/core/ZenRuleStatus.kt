 ...
 * through `ACTION_AUTOMATIC_ZEN_RULE_STATUS_CHANGED` (SPEC.md §5.8).
 *
...
/**
 * Whether Snoozemo's rule is on, read back from the platform (SPEC.md §5.8).
 *
...
/**
 * Turns a status change into a decision (SPEC.md §5.8).
 *
...
object ZenRuleStatusChange {
     ...
     * @param stillActive what the rule is doing **now**, read back rather than
     *   inferred from the event. A broadcast describes a moment that has already
     *   passed, and the rule outlives every snooze it enforces, so a delayed one
     *   can arrive after that snooze ended and another was armed — and end the
     *   *new* snooze on the old one's news, silently, since this is the ending
     *   with no notification (Codex, PR #36). A rule reporting itself active
     *   again is the evidence that the event is stale. Null where the caller has
     *   no answer, which vetoes nothing: an unreadable rule must not be the
     *   reason a snooze survives evidence that it should end.
     */
    fun resolve(
        status: ZenRuleStatus,
        ours: Boolean,
        snoozing: Boolean,
        stillActive: Boolean? = null,
    ): ZenRuleStatusAction {
        if (!ours) return ZenRuleStatusAction.None
        // Only ever a veto, and only on the endings. It cannot turn a "nothing
        // to do" into an ending, so a wrong reading here can delay an ending to
        // the next wake-up's read-back but never invent one.
        if (stillActive == true && snoozing) return ZenRuleStatusAction.None
