...
- [ ] **Open-source licenses page, linked from Settings.** (maintainer, 2026-08-23)
      Same shape as the sibling Simmo repo's `LicensesScreen`: the transitive dependency
      graph and its licenses, read from a committed `res/raw/aboutlibraries.json`
      regenerated by an AboutLibraries Gradle task, reached by a "Licenses" row at the
      foot of `SettingsScreen` beside the version and the privacy policy link. Deliberately
      not built in the PR that added the version/privacy-policy rows — a separate follow-up.
